...
class OrderFlowTestFixed extends TestCase
{
    use RefreshDatabase;
    ...
    {
        // Test with order in SELESAI status
        ...
        $response = $this->postJson("/api/v1/outlets/{$this->outlet->id}/orders/{$orderSelesai->id}/pickup");
        ...
        // Verify collected_at is set
        $orderSelesai->refresh();
        $this->assertNotNull($orderSelesai->collected_at);
    ...
    public function test_outlet_scoping_prevents_cross_outlet_access(): void
    {
        ...
        $response = $this->postJson("/api/v1/outlets/{$this->outlet->id}/orders/{$otherOrder->id}/status", [
            'to' => Order::STATUS_PROSES, // FIXED: use 'to' instead of 'status'
        ]);
